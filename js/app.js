...
Game.selectCard = function selectCard() {
  ...
  if (!Game.twoCardsChosen()) {
    // If only one card has been clicked
    return false;
  } else if (Game.twoCardsChosen() && Game.compareCards()) {
    // If two cards have been chosen check for a match with the
    // Game.compareCards function
    Game.chosenCards = [];
    return false;
  } else {
    // No match
...
Game.dealCards = function dealCards() {
  // append a three div section for each card to the Game.board and give them
  // a class='card' for the card div and ='front', ='back' for the nested divs
  var i = 0;
  ...
  // Add a click eventListener to the card and call the function selectCard
